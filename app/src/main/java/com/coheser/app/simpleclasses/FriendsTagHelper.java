...
public class FriendsTagHelper implements ClickableForegroundColorSpanCustom.OnFriendsTagClickListener {
    ...
    private void setColorForHashTagToTheEnd(int startIndex, int nextNotLetterDigitCharIndex) {
        ...
        CharacterStyle boldSpan = new StyleSpan(Typeface.BOLD);
        CharacterStyle sizeSpan = new RelativeSizeSpan(1.1f);

        if (mOnFriendsTagClickListener != null) {
            colorSpan = new ClickableForegroundColorSpanCustom(mHashTagWordColor, this);
        } else {
            // no need for clickable span because it is messing with selection when click
            colorSpan = new ForegroundColorSpan(mHashTagWordColor);
        }

        s.setSpan(colorSpan, startIndex, nextNotLetterDigitCharIndex, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        s.setSpan(boldSpan, startIndex, nextNotLetterDigitCharIndex, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
//        s.setSpan(sizeSpan, startIndex, nextNotLetterDigitCharIndex, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    private void setColorForFriendsTagToTheEnd(int startIndex, int nextNotLetterDigitCharIndex) {
        ...
        if (mOnFriendsTagClickListener != null) {
            colorSpan = new ClickableForegroundColorSpanCustom(mFriendsTagWordColor, this);
        } else {
            // no need for clickable span because it is messing with selection when click
            colorSpan = new ForegroundColorSpan(mFriendsTagWordColor);
        }
