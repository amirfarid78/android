...
import com.coheser.app.databinding.ItemSelectProductsLayoutBinding
...
class SelectProductsAdapter(
...
) : RecyclerView.Adapter<SelectProductsAdapter.MyViewholder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewholder {
        val binding = DataBindingUtil.inflate<ItemSelectProductsLayoutBinding>(
            LayoutInflater.from(parent.context),
            R.layout.item_select_products_layout,
            parent,
            false
        )
    ...
    override fun onBindViewHolder(holder: MyViewholder, position: Int) {
        ...
        if (productModel.productImage.size > 0) {
            holder.binding.productImage.controller = frescoImageLoad(
    ...
    inner class MyViewholder(var binding: ItemSelectProductsLayoutBinding) :
        RecyclerView.ViewHolder(
            binding.root
        ) {
        fun bind(position: Int, item: ProductModel?, listener: AdapterClickListener) {
            binding.addBtnTxt.setOnClickListener { v: View? ->
